...
chrome.storage.local.get(
  {
    ...
    auto_clicks: config.auto_clicks,
  ...
  (values) => {
    ...
    if (config.auto_clicks["pan.baidu.com"]) {
      let arr = config.auto_clicks["pan.baidu.com"];

      let exists = false;
      for (let item of arr) {
        if (item.path == "/share/init") {
          exists = true;
          break;
        }
      }
      exists || config.auto_clicks["pan.baidu.com"].push(bdyAutoIn);
    } else {
      config.auto_clicks["pan.baidu.com"] = [bdyAutoIn];
    }
    ...
    function load_ui(tab_id, frame_id) {
        ...
        if (index < ui_modules.length) {
          if (manifest_version === 3) {
            chrome.scripting.executeScript(
              {
                target: { tabId: tab_id },
                files: ["libs/ui/ui." + ui_modules[index++] + ".js"],
              },
              load_module
            );
          } else {
            chrome.tabs.executeScript(
              tab_id,
              { file: "libs/ui/ui." + ui_modules[index++] + ".js" },
              load_module
            );
          }
        } else {
      ...
      if (manifest_version === 3) {
        chrome.scripting.executeScript(
          { target: { tabId: tab_id }, files: ["libs/std.min.js"] },
          load_module
        );
      } else {
        chrome.tabs.executeScript(
          tab_id,
          { file: "libs/std.min.js" },
          load_module
        );
      }
    }
        ...
        for (var i = 0; i < request.items.length; ++i) {
          ...
          if (host_items) {
            ...
            for (var y = 0; y < host_items.length; ++y) {
              ...
              if (
                !(
                  host_item.selector === item.selector &&
                  host_item.path === item.path
                )
              ) {
