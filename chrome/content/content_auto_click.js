var content_auto_click = {
  ...
  match_path: function (path) {
    console.log('匹配',path)
    const u = "/share/init";
    if (path == u && content_auto_click.path.indexOf(u) != -1) {
      return true;
    }
  ...
  create_rule: function (selector) {
    ...
    var new_label = function (text) {
      return Std.ui("Label", {
        text: text,
        css: { wordBreak: "keep-all", whiteSpace: "nowrap" },
      });
    };
    var btn_cancel = {
      ui: "Button",
    ...
    content_injector.inject_css(
      "libs/ui/theme/all.css",
      shadow_root,
      function () {
